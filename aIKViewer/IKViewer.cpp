...
void IKViewer::initializeGui()
{
    ...
    TwAddVarRW(mPoseEditBar, "Max Iterations", TW_TYPE_INT32, &IKController::gIKMaxIters, " ");
...
bool IKViewer::loadMotion(const std::string& filename)
{
    mFilename = filename;
    BVHController bvhLoader;
    bool v = bvhLoader.load(filename);

    if (v)
    {
        mActor = bvhLoader.getSkeleton();
        mActor.updateTransforms();
        mIKController.setSkeleton(mActor);
        loadIKJoints();
    }
    return v;
}
...
void IKViewer::reset()
{
    loadMotion(mFilename);
    mSelectedJoint = -1;
    mGoalPosition = vec3Zero;
    glutPostRedisplay();
...
void IKViewer::draw3DView()
{
    ...
    mDrawer.draw(mIKController.getSkeleton());
...
void IKViewer::loadIKJoints()
{
    ...
    char buff[256];
    for (int i = 0; i < mIKController.getSkeleton().getNumTransforms(); i++)
    {
        Joint* joint = mIKController.getSkeleton().getByID(i);
...
void IKViewer::updateIK() // assumes joint already chosen
{
    if (mSelectedJoint == -1) return;

    //std::cout << mGoalPosition << std::endl;
    if (mType == ANALYTIC) mIKController.solveIKAnalytic(mSelectedJoint, mGoalPosition);
    else mIKController.solveIKCCD(mSelectedJoint, mGoalPosition, mIKChainSize);
}
