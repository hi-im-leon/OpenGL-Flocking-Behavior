...
void CurveViewer::initializeGui()
{
    ...
    { 
        ...
        { ASplineVec3::CUBIC_BERNSTEIN, "Berstein" },
        { ASplineVec3::CUBIC_CASTELJAU, "Casteljau" }, 
        { ASplineVec3::CUBIC_MATRIX, "Matrix" },
        { ASplineVec3::CUBIC_HERMITE, "Hermite" } 
    };
    curveType = TwDefineEnum("CurveType", curveTypeEV, 5);
...
void CurveViewer::onKeyboard(unsigned char pKey, int x, int y)
{
    ...
    {
       for (int i = 0; i < mSpline.getNumCurveSegments(); i++)
		 {
           std::cout << mSpline.getCurvePoint(i) << std::endl;
		 }
    }
